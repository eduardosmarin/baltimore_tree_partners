...
To have a more accurate detail about the tree distribution giveaways.
more about the distribution of giveaways within the cou change of
giveaways
...
table_1_tree_plantings_per_jurisdiction <-
  ...
  rename(jurisdiction = jurisdiction, `Number of Giveaways` = n, Percent = percent) |>
...
`r table_1_tree_plantings_per_jurisdiction |> filter(jurisdiction == "Baltimore City") |> pull(Percent)`of
the tree giveaways went to Baltimore City and
`r table_1_tree_plantings_per_jurisdiction |> filter(jurisdiction == "Baltimore County") |> pull(Percent)`
went to Baltimore County. However, the spread between city and
jurisdiction trees have shifted over the years. #TODO: Pull can be used
completely instead of select
