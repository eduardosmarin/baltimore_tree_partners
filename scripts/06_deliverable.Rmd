...
# weighted_neighborhood <- #TODO: Eduardo, save this out
#   interpolate_pw (
#     from = cbg                 
...
#              ))
#   )
# load("output_data/MD_Tapestry_2019_2024-09-16.csv")
# tap |>
#   st_drop_geometry() |>
...
TODO: Insert this type of map. Would avtually be very helpful maybe in a
bivariate or singular setting to see change over time.
[https://medium.com/\@lucas_bromerchenkel/choropleth-maps-with-time-sliders-using-plotly-df6e19e5f90c](https://medium.com/@lucas_bromerchenkel/choropleth-maps-with-time-sliders-using-plotly-df6e19e5f90c){.uri}
...
## a) Total households
city_cbg_2019 <-
  st_read("output_data/cbg_2019.gpkg") #CBG of 2019
...
total_tree_giveaways <-
  ...
  distinct(address) |> #TODO: Eduardo, there is an issue here if we do this...
  count() |>
  pull(n)
...
#DEXTER:
# - Should we only consider the year 2019 since that is the year of the tapestry
#   or should I consider all?
#
# - An issue noted is that using distinct, although helpful, doesn't clarify 
#   that there are addresses here that are double-counted. Like 110 Row Road and
#   110 Row Rd are counted as two separate addresses with distinct although they
#   are the same.
#
# - Is there anything else I need for this? Seems to be conducting a null hypot
```
...
## c) Combining both dataframes to do further analysis
df_odd_giveaway_tapestry <-
  city_cbg_2019_tapestry |>
  left_join(city_tree_counts_tapestry, by = "lifemode") |>
  mutate(giveaway_household_rate = giveaway_count/household_count) |>
  mutate(odds_ratio = giveaway_household_rate/base_rate_baltimore) |>
  mutate(
    se = sqrt(1 / household_count), #Found this online but is this correct for SE?                        
    ci_lower = odds_ratio - 1.96 * se,                     
    ci_upper = odds_ratio + 1.96 * se                      
  )

#DEXTER:
# - Is this the correct method to figure out the odds ratio? Unsure if I should
#   divide just by the base ratio, which I believe is what we have to do.
#   
# - The Rustic Outposts has NA for the giveaway counts so I filtered it out. 
#   First, is that what I should do for this problem? Second, is there anything
#   else I should do for this (tried looking but couldn't figure it out)?
# 
# - How do I fix so that these variables are leveled?
#
# - Is this how you calculate SE?
...
#3. Trying a boxplot instead
df_odd_giveaway_tapestry |>
  ggplot(aes(x = lifemode, y = odds_ratio)) +
  geom_boxplot(outlier.shape = NA) +                          
  geom_jitter(color = "blue", size = 3, width = 0.1) +        
  geom_hline(yintercept = 1, linetype = "dashed", color = "red") +
  labs(title = "Odds Ratio of Tree Giveaways by Lifemode Group"
         , x = "Lifemode Group"
         , y = "Odds Ratio") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
#DEXTER:
# - Why are the confidence intervals so small here?
#
# - One of the sugesstions you made was a boxplot but it is not working
# 
# - Any suggestions for improvement?
#
# - Should we look at other years besides 2019?
#
# - Trying to replicate this: 
#   https://www.fs.usda.gov/nrs/pubs/jrnl/2014/nrs_2014_locke_002.pdf
```
...
#DEXTER
# - I know I need to standardize to see an actual trend. How should I do this,
#   is it through making it for households, similar to conducting an odds ratio?
#   
# - Would I filter out to make sure there is only 2019 being taken account?
...
#TODO: Eduardo, figure out and save the shapefile of the neihgborhoods with household information to use for some standardization

#TODO: Eduardo, clean up the document since everything is finished, including the odds ratios. 
