...
graph_tree_plantings_over_years <-
  ...
  ggplot(aes(x = Year, y = `Number of Giveaways`, color = Jurisdiction)) +
   ...
   geom_point() +
    ...
    labs(title = "Total Giveaways by Years for each Jurisdiction", #TODO: Better title needed for this 
         x = "Year",
         y = "Total Giveaways",
         color = "Jurisdiction") +  
    theme_minimal()
...
graph_tree_plantings_over_years_seasons <-
  ...
  ggplot(aes(x = Year, y = `Number of Giveaways`, color = Jurisdiction)) +
   ...
   geom_point() +
    ...
    scale_color_manual(values = jurisdiction_colors) +  
    labs(title = "Total Giveaways by Years and Season for each Jurisdiction", #TODO: Better title needed for this 
         x = "Year",
         y = "Total Giveaways",
         color = "Jurisdiction") +  
    theme_minimal() +
...
compared to `r d_tree_plantings_over_years_seasons |> filter(Year == 2023 & Season == "Spring" & Jurisdiction == "Baltimore County") |> pull("Number of Giveaways")`.
...
Given that Blue Water Baltimore's tree giveaways are primarily concentrated within Baltimore City for most years, totaling [insert number] giveaways over the past ten years. We will examine Baltimore City more closely at the neighborhood level to better examine the distribution of tree giveaways. Through this process, we will uncover if there are any spatial patterns throughout the years and if distributions have been changing significantly.
...
As previously noted, a significant proportion of total tree plantings has been concentrated in Northeast, Central, and Southeast Baltimore. Specifically, the top ten neighborhoods with the highest number of tree plantings are listed in descending order: `r figure_4_tree_neighborhoods_layer |> arrange(desc('Number of Trees')) |> slice(1:9) |> pull(Name) |> paste(collapse = ", ")`, followed by
...
#TODO: Dexter, confused why 	Johns Hopkins Homewood is showing although it technically has nothing listed for its Trees per Household
```
...
However, evaluating neighborhoods solely based on the giveaway count may not provide a comprehensive view, as area sizes can vary significantly among them.
...
#Maybe make a graph from Post-2020 COVID and see if and have different of the overall change. Percent of total distribution for that. 
#TODO: Eduardo, do that above to standardize and see more vividily the changes

#TODO: Dexter, unsure, how to get the count of households since the units are neighborhoods and not census blocks (don't know how to standardize). Also, is there a better visual here for this?
...
graph_plantings_maryland_year_season <-
  ...
  ggplot(aes(x = Year, y = `Number of Giveaways`, fill = Season)) +
   geom_bar(position = "dodge", stat = "identity") +
    ...
    scale_y_continuous(
      limits = c(0, 1500),  #TODO: Make to 1750 the upper limit (Finished)   
      breaks = seq(0, 1500, by = 250),  
      labels = scales::comma  
    ) +
    scale_x_continuous (
...
ggsave(
  filename = file.path("../baltimore_tree_partners/deliverables", "location_graph_1_tree_plantings_general_v1.png"), #TODO: Fix to make more general file saving path
...
reach **pre-pandemic levels.** Since the number of tree plantings within the city reached 
...
`r unique(unique_provider_na$n)`. This approximately accounts for`r unique(unique_provider_na$percent)`% of the dataset. 
...
graph_4_giveaway_locations <-
  ...
  ggplot(aes(x = reorder(`Giveaway Location`, `Number of Trees`), y = `Number of Trees`)) +
    ...
    theme_minimal() +
    scale_y_continuous(expand = c(0, 0)) + #(TODO: Eduardo, USE THIS FOR THE WATERSHED GRAPH)
    NULL
...
#TODO: Eduardo, choose beter colors
```
...
# ggsave(
#   filename = file.path("../baltimore_tree_partners/deliverables", "species_graph_5_common_rare_species.svg"), #TODO: Fix to make more general file saving path
...
ggsave(
  filename = file.path("../baltimore_tree_partners/deliverables", "species_graph_6_top_ten_species_jurisdiction.png"), #TODO: Fix to make more general file saving path
...
#### Income 
Figure 1. Synced Map of Household Income and Giveaway Count (Per Census
Blocks)
TODO: Check how you are defining the giveaway counts if by household or by area (should be household)
TODO: Figure out how many trees are not in block groups with NA
```{r}
...
Graph 1. Regression Income Graph of Income and Count
```{r}
regression_income_count_graph <-
  ...
  labs(
    ...
    y = "Giveaway Count" #TODO: First, figure out if the number is true about 3k points not being captured since they are NA. Put in caption that these are not included and the amount 
  ) +
  scale_x_continuous(
...
Graph 2. Ridgeline Graph of Income over Year
```{r}
...
Graph 3. Boxplot of the Income over Year
```{r echo=TRUE}
boxplot_income_years <-
  ...
  NULL #TODO: Dexter, means find statistical significance

boxplot_income_years
```

Figure 2. Bivariate Map of Household Income and Giveaway Count (per
Census Block)

TODO: Lawrence Brown, Black Butterfly, White L
```{r}
## CBG Bivariate
cbg_census_count <-
  cbg |>
...
TODO: Insert this type of map. Would avtually be very helpful maybe in a bivariate or singular setting to see change over time. https://medium.com/@lucas_bromerchenkel/choropleth-maps-with-time-sliders-using-plotly-df6e19e5f90c

#### Education

Figure 1. Synced Map of Highschool Education and Giveaway Count (Per Census
Blocks)
...
Graph 1. Regression Education Graph and Count
```{r}
...
Graph 2. Ridgeline Graph of Education over Year
```{r}
...
Graph 3. Boxplot of the Education over Year
```{r echo=TRUE}
...
Figure 2. Bivariate Map of Education and Giveaway Count (per
...
Figure 1. Synced Map of Race and Giveaway Count (Per Census
Blocks)
...
Graph 1. Regression Race Graph and Count
```{r}
...
Graph 2. Ridgeline Graph of Race over Year
```{r}
...
#TODO: Look at what low-income, high_
# Group by location, and location year, get the mean of the median household
# Get the mean of the education as well.
# Get a tree level file, every row is a tree, you have a column of where the giveaway happened, and the year, and you have a column for the median household income to where the tree went to alongside the education of where the tree went home to
# Take that file group_by (location, by_year) |> summarize() |> mean = mean(education) DEXTER: list(mean_cl_normal) ~ makes a nested datafraem |> unnnest()
```

Graph 3. Boxplot of the Race over Year
```{r include=FALSE}
boxplot_race_years <-
  blue_water_giveaway_data |>
  mutate(pct_white_alone = 100*(white_alone/total_population)) |>
...
Figure 2. Bivariate Map of Race and Giveaway Count (per
Census Block) ~ Baltimore City
```{r}
## CBG Bivariate
cbg_census_count <-
...
Figure 3. Bivariate Map of Race and Giveaway Count (per
Census Block) ~ Baltimore County
```{r}
## setting the bivariate count and race
bivariate_count_race <-
...
#     rename(NAME = NAM) |>
#     mutate(
#       pct_at_least_hs = (100)*((rowSums(across(starts_with("ed_"))))/total_population_over_25), 
#       pct_at_least_college = (100)*((rowSums(across(starts_with("ed_plus"))))/total_population_over_25) 
#     )|> 
#     dplyr::select(-starts_with("ed_")) |>
#     st_write("output_data/cbg_2019.gpkg")
...
```{r}
#Getting the number of giveaway in each
tree_giveaway_2019_city <-
  blue_water_giveaway_data |>
  st_drop_geometry() |>
  filter(year == 2019 & jurisdiction == "Baltimore City") |>
  distinct(address) |>
  count() |> #making sure that we only get the unique addresses given
  pull(n) #now a vector

tree_giveaway_2019_county <-
  blue_water_giveaway_data |>
  st_drop_geometry() |>
  filter(year == 2019 & jurisdiction == "Baltimore City") |>
  distinct(address) |>
...
#Caluclating rates (for odd ratios)
tree_giveaway_2019_city <- 
  tree_giveaway_2019_city/households_2019_city

tree_giveaway_2019_county <- 
  tree_giveaway_2019_county/households_2019_county

tree_giveaway_2019 <-
  (tree_giveaway_2019_city + tree_giveaway_2019_county) / (households_2019_city + households_2019_county)
```


```{r}
#Trees by lifemode group
tree_giveaways_by_lifemode <- 
  bwb_total_giveaway_with_Tapestry_2019 |>
  filter(year == 2019) |>
  group_by(lifemode) |>
  summarise(trees_given = n()) 

#Finding the total population of each group
population_by_lifemode <- 
  bwb_total_giveaway_with_Tapestry_2019 |>
  group_by(lifemode, jurisdiction) |>
  summarise(total_population = sum(total_population, na.rm = TRUE)) |>
  filter(!is.na(jurisdiction))

#^ This doesn't work since it is double counting. I need the original shapefile of the data to get the counts per each group 

#Eduardo, Normalizing the populations and the odds ratio. Have a boxplot (instead of total number). Out of all the trees given away, and divide by the populations. Let's only look at 2019 giveaways. And then see. A ratio of ratios. An odds ratio. We want to know if 10% of trees went to affluent estate, and 10% of the population is affluent estates. The odd ratio is 1. Let us say that 20% of trees but only went to the affluent estate, the odd ratio is 2. This makes more sense #We want an estimate that takes into account trees per lifemode group (number of people per life group) ~~ Just 2019. Maybe use all of the years later.

#I cannot determine currently what the amount of group is in each sector of Baltimore City and County, once I have that shapefile, I believe I should be able to do that
...
#TODO: Dexter (What is the pois.exact() function)

#Read through the paper and the Phake city example made sense
```

Graph 1. Lifemode Group and Jurisdiction
```{r}
lifemode_colors <- c(
...
Group 2. Lifemode Group and Total Giveaway Number
```{r}
bwb_total_giveaway_with_Tapestry_2019 |>
  st_drop_geometry() |>
  group_by(lifemode) |>
...
Graph 2. Lifemode Group and Every Year (Has there been changes to the types of people?)
```{r}
...
#TODO: Search for snakey diagram
Graph 3. Lifemode Groups in Top 10 Giveaway Locations
```{r}
...
#More information
```{r}
#Go ahead to look at the example of phake City. What is the overall rate out of the population. Out of the entrie population, how many trees were given out. Total number of trees/Total number of household (We need that since that is the benchmark. We need to know the overall rate). We want to compare the rate:

#TODO: Eduardo, how different is what we observed with the overall rate. 

#TODO: Eduardo, fit a generalized linear model and your outcome would be a giveaway. You need to get the background rate (overall) in there. ~ look into Epidemeology (instead of trees, you want cases) ~ Look at the Epidimeology Textbook of R

#TODO: Eduardo, look at Hard Word, Easy Place paper

#Would page 13 be worth it?

# https://www.fs.usda.gov/research/treesearch/49227

```

Graph 4. Top 5 Species for Each Lifemode Group
```{r}
bwb_total_giveaway_with_Tapestry_2019 |>
  st_drop_geometry() |>
...
Graph 5. Tapestry Box Plot (Compared to Affluent Estates)
...
bwb_total_giveaway_with_Tapestry_2019 |> 
  ...
  222# https://rpkgs.datanovia.com/ggpubr/reference/stat_compare_means.html
```


Figure 1. Map of Tapestry Groups in Baltimore
```{r}
baltimore_2019_layer
```
