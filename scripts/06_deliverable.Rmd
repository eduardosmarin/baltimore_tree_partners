...
neighborhood_read <-
  st_read("/Users/eduardosmarin/Desktop/baltimore_tree_partners_data_to_big/shapefiles/Neighborhood/Neighborhood.shp") |>
...
blue_water_giveaway_data <-
   st_read("/Users/eduardosmarin/Desktop/baltimore_tree_partners/output_data/blue_water_baltimore_data_2024-08-21.gpkg") #FINAL GEOPACKAGE!:)
...
blue_water_watershed_data <-
 st_read("/Users/eduardosmarin/Desktop/baltimore_tree_partners_data_to_big/shapefiles/Maryland_Watersheds_-_8_Digit_Watersheds/HYDR_Watersheds8Digit_DNR.shp") |>
 st_transform(4326) 
...
server <- function(input, output, session) {
  ...
  filtered_data <- reactive({
    ...
    # Pause execution here to inspect the reactive context
    browser()
  ...
  output$filtered_map <- renderMapview({
    data <- filtered_data()  # This should only be called inside renderMapview
    
    # Pause execution here to inspect before rendering the map
    browser()
...
neighborhood_read |>
  left_join(
    read_csv("/Users/eduardosmarin/Desktop/baltimore_tree_partners/output_data/neighborhood_count_bwb.csv")
    , by = "Name"
  ) |>
  mutate(count = case_when(
    is.na(count) ~ 0,
...
figure_4_tree_neighborhoods_layer |>
  st_drop_geometry() |>
  left_join(
    read_csv("/Users/eduardosmarin/Desktop/baltimore_tree_partners/output_data/neighborhood_count_year_bwb.csv"),
    by = "Name"
  ) |>
...
cbg_census_count <-
  cbg |>
  left_join(
  read_csv("/Users/eduardosmarin/Desktop/baltimore_tree_partners/output_data/census_count_year.csv")
    , by = "NAME"
  ) 
...
ggplot(aes(x = household_income, y = factor(year), fill = `Number of Giveaways`)) +
  geom_tile() +
  scale_fill_gradient(low = "white", high = "red") +  # Adjust colors as desired
  labs(
