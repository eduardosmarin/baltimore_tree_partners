...
#TODO: Save this out the geopackage for this 
variables_acs <- 
...
## NEEDS TO GET FIXED
ui <- fluidPage(
  titlePanel("Tree Plantings Map"),
  sidebarLayout(
    sidebarPanel(
      selectInput("year", "Select Year:", 
                  choices = unique(blue_water_giveaway_data$year), 
                  selected = unique(blue_water_giveaway_data$year)[1]),
      selectInput("season", "Select Season:", 
                  choices = unique(blue_water_giveaway_data$season), 
                  selected = unique(blue_water_giveaway_data$season)[1])
    ),
    mainPanel(
      mapviewOutput("filtered_map")
    )
  )
)
...
Taking a step back, next, we will observe the general trends over the
years across the entire state of Maryland, where these giveaways have
occurred.

**Graph 3. Graph of Tree Plantings in Maryland (per Year and Season)**
...
Given that Blue Water Baltimore's tree giveaways are focused within
Baltimore City, the following analysis will examine tree distributions
specifically at the **neighborhood level** within the city.
...
As alluded to earlier, there is a larger percentage of tree plantings
within the northern and central part of Baltimore. Neighborhoods like
...
# `r paste0(figure_4_tree_neighborhoods_layer) |> arrange(desc('Number of Trees')) |> slice(1:4) |> pull(Neighborhood), collapse (", ")` and `r paste0(figure_4_tree_neighborhoods_layer) |> arrange(desc('Number of Trees')) |> slice(5) |> pull(Neighborhood)`.
```
