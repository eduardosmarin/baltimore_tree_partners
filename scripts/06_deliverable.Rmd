...
blue_water_giveaway_data <-
   st_read("../baltimore_tree_partners/output_data/blue_water_baltimore_data_2024-08-23.gpkg", as_tibble = TRUE) #FINAL GEOPACKAGE!:) # TODO add as_tibble everytime you do st_read
...
Given that Blue Water Baltimore's tree giveaways are primarily concentrated within Baltimore City for most years, totaling `r d_tree_plantings_over_years_seasons %>% filter(Jurisdiction == "Baltimore City") |> summarise('Number of Giveaways' = sum('Number of Giveaways')) |> pull('Number of Giveaways)'` giveaways over the past ten years. We will examine Baltimore City more closely at the neighborhood level to better examine the distribution of tree giveaways. Through this process, we will uncover if there are any spatial patterns throughout the years and if distributions have been changing significantly.
...
As previously noted, a significant proportion of total tree plantings has been concentrated in Northeast, Central, and Southeast Baltimore. Specifically, the top ten neighborhoods with the highest number of tree plantings are `r figure_4_tree_neighborhoods_layer |> arrange(desc(Number of Trees)) |> slice(1:9) |> pull(Name) |> paste(collapse = ", ")`, followed by `r figure_4_tree_neighborhoods_layer |> arrange(desc(Number of Trees)) |> slice(10) |> pull(Name)`, listed in descending order.
...
# TODO: put in chunk zero
library(ggridges)

blue_water_giveaway_data |> 
  st_drop_geometry() |> 
  mutate(year = factor(year, levels = c(2023:2013))) |> 
  ggplot(aes(median_household_income, year, year, fill = year)) +
  # TODO; go nutz, but tell a sstory
  # https://jtr13.github.io/cc19/ridgeline-plots.html
  geom_density_ridges(jittered_points = TRUE) + 
  theme_bw(16) + 
  scale_x_continuous(labels = scales::dollar_format()) + 
  theme(legend.position = "none") +
  labs(title = 'The distirbution of income is getting lower over time') + 
  NULL
...
blue_water_giveaway_data |>
  mutate(household_income = cut(
    ...
    breaks = c(seq(0, 200000, by = 25000), Inf),
    labels = c(paste0("$", scales::comma(seq(0, 180000, by = 25000)), 
                      " - $", scales::comma(seq(25000, 200000, by = 25000))),
               "$200,000+"),
    include.lowest = TRUE,
    right = FALSE
  )) |>
  ...
  st_drop_geometry() |>
  ggplot(aes(x = household_income, y = `Number of Giveaways`, color = factor(year))) +
  geom_point() +
  facet_grid(. ~ year) +
  labs(
    ...
    y = "Giveaway Count",
    color = "Year",
    title = "Scatterplot of Giveaway Count vs Median Household Income by Year"
  ) +
  theme_minimal() +
...
blue_water_giveaway_data |>
  ...
  filter(!is.na(household_income)) |>
  st_drop_geometry() |>
ggplot(aes(x = household_income, y = factor(year), fill = `Number of Giveaways`)) +
  geom_tile() +
  scale_fill_gradient(low = "white", high = "red") + 
  labs(
    x = "Median Household Income Group",
    y = "Year",
    fill = "Total Giveaways",
    title = "Heatmap of Giveaway Count vs Median Household Income by Year"
  ) +
  theme_minimal() +
...
figure_3_census_count_layer <-
  ...
  filter(!is.na(tree_per_household)) 
...
library(ggpubr) # TODO add to chunk zero
...
bwb_total_giveaway_with_Tapestry_2019 |> 
  ...
  # drop_na() |> 
  # filter(lifemode != 'Rustic Outposts') |> 
  # ggplot(aes(lifemode, n)) +
  ggpubr::ggboxplot( x= 'lifemode', y ='n'
...
```{r}
## Two different ways to do the slice above and blow
