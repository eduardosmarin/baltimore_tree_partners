...
my_vars <- c(  'pct_median_household_income' = 'B19013_001'
...
blue_water_baltimore_data_check <- 
  st_read("output_data/blue_water_baltimore_data_2024-08-22.gpkg") #DEXTER: Change whenver your sys.date does it

blue_water_baltimore_data_check |> glimpse()
blue_water_baltimore_data_check |> summary()
blue_water_baltimore_data_check |> map(~levels(.))
blue_water_baltimore_data_check |> mapview(zcol = 'pct_median_household_income')
...
# TODO, give "itree" a better name like "itree_sp_code" or similar (COMPLETED)


# TODO, fix missing State values (COMPLETED)
blue_water_baltimore_data_check |> st_drop_geometry() |> tabyl(city, state) # missing state values (COMPLETED)
...
blue_water_baltimore_data_check |> st_drop_geometry() |> tabyl(giveaway_location)
...
blue_water_baltimore_data_check %$% cor.test(at_least_hs, at_least_college)

blue_water_baltimore_data_check %$% cor.test(pct_median_household_income, at_least_college) # good.

blue_water_baltimore_data_check |> 
  ggplot(aes(pct_median_household_income)) + 
  geom_density() # good
