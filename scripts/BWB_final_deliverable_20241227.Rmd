...
subtitle: "An Analysis of 10+ Years of Blue Water Baltimore Giveaway Data Using R"
author: "Eduardo Marin and Dexter H Locke"
date: "October, 2024"
output:
...
The data was consolidated from various sources, including Excel spreadsheets, CSV files, and shapefiles spanning from 2013 to 2023. After extensive data wrangling, the information was successfully integrated into a single single geopackage (\*.gpkg), which was returned to Blue Water Baltimore.

This data reconciliation effort of Blue Water Baltimore is part of a broader discussion on data integrity and collaboration among partner organizations, such as TreeBaltimore, Baltimore City's urban and community forestry umbrella organization, and Baltimore Tree Trust, an environmental nonprofit focused on urban forestry in Baltimore. Through these partnerships, we aim to facilitate seamless data sharing among organizations to address the region's environmental challenges more effectively.
...
This work builds on the foundation laid in an earlier report, ["A Market Analysis of Opt-In Tree Planting and Rain Barrel Installation in Baltimore, MD, 2008–2012"](https://research.fs.usda.gov/sites/default/files/2023-05/locke_morgan_2015_a_market_analysis_of_opt-in_tree_planting_and_rain_barrel_installation_in_baltimore_md_2008_-_2012.pdf)*,* and peer-reviewed paper, ["Doing the hard work where it's easiest? Examining the relationships between urban greening programs and social and ecological characteristics."](#0)

Similar research has been conducted in

Philadelphia: [A Market Analysis of TreePhilly’s Yard Tree Program, Spring 2012—Spring 2014](https://treephilly.org/wp-content/uploads/2015/03/MarketAnalysis_Report_TreePhillyGiveaways20150120.pdf)

New York City: [A Market Analysis of New York Restoration Project’s Tree Giveaway Program, Spring 2008—Fall 2013](https://dexterlocke.com/wp-content/uploads/2023/01/giveawaymarketanalysis_d_locke_m_mitchell_june2014.pdf)

with some cross-site synthesis emerging in the peer reviewed literature "[Branching out to residential lands: missions and strategies of five tree distribution programs in the U.S.](https://research.fs.usda.gov/treesearch/53733)" and in an Arborist News article, "[Best practices for yard tree distribution programs](https://research.fs.usda.gov/treesearch/56979)".
...
# load('output_data/bwb_total_giveaway_with_Tapestry_2019_2024-09-16.RData')
load('output_data/bwb_total_giveaway_with_Tapestry_2019_2025-04-03.RData')
...
Please note that `r scales::comma(unique(unique_provider_na$n))` trees or `r round(unique(unique_provider_na$percent), 2)`% do not have a giveaway location recorded.
...
As anticipated, there is a high concentration of plantings in Baltimore County and Baltimore City over the past decade where giveaways occur, with fewer plantings occurring in peripheral areas, including Howard, Anne Arundel, Carroll, and Harford counties where BWB does not operate.
...
-   In years 2018 and 2019, Blue Water Baltimore and TreeBaltimore partnered on their Fall giveaways. The values are were unusually high because they combined their counts together.
...
In addition to parsing giveaways by seasons per year, it is helpful to examine where the trees have gone over time, by jurisdiction. There were more giveaways in Baltimore City in years 2018 and 2019 than Baltimore County, largely due to the previously mentioned partnership with Tree Baltimore. Since the pandemic there has been a steady increase in County giveaways.
...
In the case of the amount of tree giveaway locations, the most giveaways originated at Stillmeadow Community Fellowship. **However, please note that `r scales::comma(unique(unique_provider_na$n))` trees or `r round(unique(unique_provider_na$percent), 2)`% do not have a giveaway location recorded.**
...
table_8_watershed_count_datatable <-
  ...
  arrange(desc(n)) |>
  mutate(  Percent = round(100 * percent, 2)
         , `Valid Percent` = round(100 * valid_percent, 2)
         ) |>
  mutate(Watershed = case_when(
    is.na(watershed) ~ "Outside BWB Service Area",
    TRUE ~ watershed
  )) |>
  select(Watershed, Count = n, Percent, `Valid Percent`)
...
For this part of the analysis, we will look at median household income at the block group scale to see if it has any correlation with number of giveaways. In terms of the scale, we will look specifically at the census block group scale to determine this. We will first look at synced maps.
...
The spatial association between giveaways per household and income is not strong. There is apparently weak relationship visible from these two maps.
...
Statistically, the relationship is also weak (Pearson's r = `r correlation_income_household_count$estimate`) and significant (p-value = `r correlation_income_household_count$p.value`), as shown below in the scatterplot with a linear regression.
...
We can also visualize giveaway counts versus income per year. The above is a ridgeline graph showing the distribution of giveaways versus median household income. Lumps to the right indicate higher incomes while lumps to the left indicate lower income. Overall, the median (quantile 3) is shifting left and the distribution is becoming more flatter.

## Tapestry

### General

Rather than analyze a single socio-demographic or economic variable like median household income, it can be useful to categorize places based on multiple dimensions simultaneously. Geo-demographic market segmentation is a suite of techniques ideally suited categorizing neighborhoods or Census block groups. Briefly, multiple characteristics are clustered to form these categories. Although not all individuals will match the profile exactly, the mix of individuals in the same category will be similar. We used ESRI's Tapestry geo-demographic segmentation dataset here. For more details, please read the following [link](https://www.esri.com/en-us/arcgis/products/data/data-portfolio/tapestry-segmentation).
...
Above demonstrates that some segments received more free trees than others. But these counts per category do not take into account that the number of households per category also varies. By taking the ratio of the two fractions: 1 the fraction of households per category that receive a free trees, and 2 the fraction of households in that category, we can calculate the odds ratio. Values between 0 and less than 1 mean that fewer giveaway trees were received than if giveaway trees were proportional to the number of households in that Tapestry segment. Values greater than 1 mean that there were more giveaways than would have been expected under a scenario where both proportions were the same. These analyses only pertain to year 2019 giveaway data to correspond to the year 2019 Tapestry classification.
...
join_tapestry_cbg <-
  read_csv("output_data/MD_Tapestry_2019_2025-04-03.csv") |> # TODO get level DHL tk
...
city_cbg_2019_tapestry <- 
  ...
  st_drop_geometry() |>
  group_by(lifemode) |> 
  summarize(household_count = sum(households, na.rm = TRUE)) |>
  filter(!is.na(lifemode)) 
...
The analysis of tree giveaway distributions using odds ratios shows no significant association with the likelihood of receiving trees and their respective tapestry group for most groups. Exceptions includes Middle Ground and Uptown Individuals who have higher odds ratios, suggesting they are more likely to receive trees than their population percentage. Conversely, Senior Styles, Family Landscapes, Upscale Avenues, and Affluent Estates have lower odds of having received a tree. Overall, however, this where and for whom Blue Water's Baltimore are working to increase tree canopy cover.

# Conclusion

This preliminary analysis of over a decade of Blue Water Baltimore’s tree giveaway data reveals patterns and trends in Baltimore City and Baltimore County. By investigating patterns in tree distribution across locations, species, watershed areas, and socioeconomic factors, we gain valuable insights into the program’s impact on promoting urban forestry through the distribution of trees on residential lands. Ongoing collaboration with Blue Water Baltimore seeks to refine these data and analytics to support their decision making and goals to improve public understanding and engagement.
...
Eduardo Marin, an intern with the USDA Forest Service Baltimore Field Station, worked alongside Dexter Locke, a Research Geographer with the USDA Forest Service Baltimore Field Station, to acquire, standardize, integrate, reconcile, and visualize these data. Eduardo then conducted theses analyses and created the presentation with Blue Water Baltimore.
