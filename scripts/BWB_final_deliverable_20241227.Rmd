...
**Please note that `r unique(unique_provider_na$n)` trees or `r round(unique(unique_provider_na$percent), 2)`% do not have a giveaway location recorded.**
...
As anticipated, there is a high concentration of plantings in Baltimore County and Baltimore City over the past decade where giveaways occur, with fewer plantings occurring in peripheral areas, including Howard, Anne Arundel, Carroll, and Harford counties where BWB does not opperate.
...
data_regression_income_graph %$% 
  ...
  broom::tidy() # |>  mutate_if(is.double, round, 3)
```

The spatial association between giveaways per household and income is not strong. There is apparently weak relationship visible from these two maps. Statistically, the relationship is also weak (`r correlation_income_household_count$estimate`) but significant (p-value = `r correlation_income_household_count$p.value`), as shown below in the scatterplot with a linear regression.
...
Above demonstrates that some segments received more free trees than others. But these counts per category do not take into account that the number of households per category also varies. By taking the ratio of the two fractions: 1 the fraction of households per category that receive a free trees, and 2 the fraction of households in that category, we can calculate the odds ratio. Values between 0 and 1 mean that fewer giveaway trees were received than if giveaway trees were proportional to the number of households in that Tapestry segment. Values greater than one mean that there were more giveaways than would have been expected under a scenario where both proportions were the same.
