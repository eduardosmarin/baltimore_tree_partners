...
The document was prepared by Dexter H. Locke and Eduardo Marin. The research document will include the following sections with accompanying downloadable visuals. 
- Location
- Time
- Species
- Watershed
...
# setting for get_acs
# census_api_key('58fc555c77c229747ade7d9fe50e7c71297cf91a', install = TRUE, overwrite = TRUE)
>>>>>>> f2a816223faf3c86266659b4c09c19b6422407e6
...
Dark Blue: #1D4F8B
Green: #6AB346
Intermediate Blue 1: #5A99D1
Intermediate Blue 2: #2F6A9E
Light Green: #8BCF6A
Dark Green: #4E7E2B
...
Light Green: #8BCF6A
Dark Green: #4E7E2B
Very Light Blue: #A0D4F5
Very Dark Blue: #0F3658
Very Light Green: #A4D88A
Intermediate Green 1: #78C265
Intermediate Green 2: #5DA84A
```
...
state_layer <-
 maryland |>
 mapview(col.regions = "#6FB1E3", alpha = 0.25, layer.name = "Maryland") 

## Making the Baltimore County layer
county_layer <-
  baltimore_city_counties |>
  mapview(col.regions = "#2F6A9E", alpha = 0.25, layer.name = "Baltimore County") 
...
city_layer <-
  ...
  mapview(col.regions = "#1D4F8B", alpha = 0.25, layer.name = "Baltimore City") 
...
neighborhood_layer <-
  ...
  st_transform(4326) |>
  mapview(col.regions = "#0F3658", zcol = "Name", layer.name = "Baltimore's Neighborhood")
...
giveaway_layer <-
  blue_water_giveaway_data |>
  mapview(col.regions = "#6AB346", alpha = 0.25, layer.name = "Total Tree Giveaways") 
...
## Making the Counties Layer
county_colors <- c(
  "Baltimore City" = "#1D4F8B",
  "Baltimore County" = "#2F6A9E", 
  "Neither" = "#6FB1E3"
)
...
In the past eleven years of running Blue Water Baltimore's giveaway program, there has been a total of `r nrow(blue_water_giveaway_data)` tree giveaways. The number may be an overestimate since 2017 and 2018 are counted. These years were when Blue Water Baltimore partnered with TreeBaltimore. Even so, these tree plantings have reached homeowners across the state, from Frederick to Caroline County.

Figure 1. Giveaway Map of Baltimore County
```{r echo=FALSE}
giveaway_layer + state_layer #TODO: Fix the map zoom to focus on Maryland
```

Of course, most of these tree plantings focus on homeowners within the city of Baltimore and Baltimore County. #TODO: Change the verbiage here

Figure 2. Giveaway Map of Baltimore County
```{r}
bwb_total_giveaway |>
mapview(zcol = "bwb_total_giveaway",  # Column with count or density
...
bwb_total_giveaway |>
mapview(zcol = "value", map.types = "OpenStreetMap", 
