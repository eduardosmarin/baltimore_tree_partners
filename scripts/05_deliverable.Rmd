...
- Watershed (0/1)
  ...
  - Have they been increasing in some place and decreasing in others
  
  - Line chart of the giveaways for the watershed (same as county and city)
...
blue_water_giveaway_data |>
  ...
  arrange(desc(n)) |>
  rename(Count = n) |>
  mutate(`Watershed Name` = case_when(
    is.na(`Watershed Name`) ~ "Outside BWB Service Area",
    TRUE ~ `Watershed Name`
  ))
```
...
Graph 8. Number of counts per watershed per year
```{r}
graph_8_watersheds_count <-
  ...
  st_drop_geometry() |>
  group_by(season, year, `Watershed Name`) |>
  rename(`Watershed` = `Watershed Name`) |>
  summarise(`Number of Giveaways` = n(), .groups = 'drop') |>
  rename(Season = season, Year = year) |>
  mutate(Year = as.double(Year)) |>
...
#TODO: make sure the code isn't the, just write the ggsave (don't save it out and delete the old ones, and the news ones, the number might have to change)
```
