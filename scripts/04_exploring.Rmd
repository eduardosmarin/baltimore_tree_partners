...
tap_query <- "SELECT ID, ST_ABBREV, TURBZNAME, TURBZCODE, TLIFENAME, TLIFECODE, TSEGNAME, TSEGNUM THHBASE from TAP19_HH_bg WHERE ST_ABBREV = 'MD'"
...
tap <-
  ...
  rename(GEOID_2019 = ID, households = THHBASE) |>
...
load('output_data/tap_lu_2024-09-16.RData')
...
list.files("Maryland_Watersheds_-_8_Digit_Watersheds")

(
  sheds_all <- 
  st_read('Maryland_Watersheds_-_8_Digit_Watersheds/HYDR_Watersheds8Digit_DNR.shp'
          , as_tibble = TRUE)
  )
...
sheds <- 
  sheds_all |> 
  filter(mde8digt %in% sheds_to_keep) 
