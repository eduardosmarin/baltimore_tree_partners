...
(cbg <-
  ...
  # , edu_base = 
  # , edu_attain = (highschool or greater / edu_base)*100
  # )
```
...
bwb_jurisdiction_years_long |>
  ...
  ggplot(aes(x = Year, y = number_of_trees, fill = county)) +
    ...
    scale_x_continuous ( # TODO Graphs need adjustment like this
      breaks = seq(2013, 2023) 
    ) +
    scale_y_continuous(
      limits = c(0, 1500),         
...
neighborhood_count_layer <- # TODO - my HTML was large if I cut that off on 508 and 517, is it storing the object
neighborhood_read |>
  left_join(
    read_csv("output_data/neighborhood_count_bwb.csv")
    , by = "Name"
  ) |>
  mapview(zcol = "count", layer.name = "Giveaway Count per Neighborhood", at = legend_order_neighborhood_count) # TODO - blank needs to be zeros called NA (if it doesn't have successful join, would it run faster without)
...
bwb_giveaway_neighborhood_dt <- # TODO - more code making it slower. Probably affect our storage. Sometimes you will need to make lots objects (not this time). Refactoring is the prototype (does what it does, rewriting the results of the same outcome (saves speed and memory))
  bwb_giveaway_neighborhood |>
  DT::datatable()
...
```{r echo=FALSE}
# Calling the datatable
bwb_giveaway_year_neighborhood_dt # TODO - do this
```
...
#Making the read for the giveaway location points
bwb_giveaway_location_read <-
  bwb_giveaway_location |>
    left_join(bwb_location_tribble, by = "giveaway_location") |>
    st_as_sf(coords = c("entry_long","entry_lat"), crs = 4326)

#Making the layer of the origin points
giveaway_location_layer <-
  bwb_giveaway_location_read |>
  mapview(zcol = "giveaway_location", color = giveaway_location_color, layer.name = "Giveaway Locations")

#The combined map
giveaway_location_link <-
  link_layer + giveaway_location_layer # TODO st_distance is how to get the distance between two things. Do this for fun (make a graph)
