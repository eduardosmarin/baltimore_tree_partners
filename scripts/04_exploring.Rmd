...
## A) Map of Tree Giveaways Each Year
...
## B) Interactive Total Plantings Each Year Table
...
## C) Bar Chart of Total Plantings Each Year (Divided by Season)
...
## D) Line Chart of Total Plantings Each Year (Divided by Season)
...
## a) Map of the Three Jurisdictions
```{r include=FALSE}
...
## b) Interactive Plantings in Each Jurisdiction Table
...
## c) Plantings in Each Jurisdiction in Each Year Data Table
...
## d) Plantings in Each Jurisdiction in Each Year Graph
...
## e) Planting Number in Each Neighborhood of Baltimore Map

```{r}
# Finding the number of trees giveaway within each neighborhood
  bwb_total_giveaway |>
...
## f) Plantings in Each Neighborhood Data Table

```{r}
# Creating the neighborhood data frame
bwb_giveaway_neighborhood <-
  neighborhood_count_read |>
  st_drop_geometry() |>
...
## g) Top 20 Plantings of Neighborhood Graph
...
## h) Plantings in Each Neighborhood and Year Data Table

```{r}
# Finding the number of trees giveaway within each neighborhood and year
neighborhood_count_year_intersect <-
  st_intersection(bwb_total_giveaway, neighborhood_read) |>
  st_drop_geometry() |> 
  group_by(Name, year) |>
  summarise(count = n())

neighborhood_count_year_read <-
left_join(neighborhood_read, neighborhood_count_year_intersect, by = "Name") |>
   mutate(count = ifelse(is.na(count), 0, count))

# Creating the neighborhood and year datagrame
...
# i) Giveaway Location Data Table
```{r}
...
# j) Giveaway Location Data Graph
```{r}
bwb_giveaway_location |>  
...
# k) Planting Number in Each Census Tract of Baltimore and Baltimore County Map

```{r}
# Finding the number of trees giveaway within each census block
census_count_intersect <-
...
census_count_read <-
left_join(cbg, census_count_intersect, by = "NAME") |>
   mutate(count = ifelse(is.na(count), 0, count))
...
# l) Synced map of Hosehold Income and Giveaway Count
...
## c) Common Name Graph (Both Rare and Common)
