...
- Option Shift K gives a list of all of the shortcuts in RStudio
...
(MD <-
  tidycensus::get_acs(
   ...
   st_transform(crs = st_crs(bwb_total_giveaway)))
...
# 5 augment tree data with census
```{r}

bwb_total_giveaway_aug <- 
  bwb_total_giveaway |> 
  st_join(cbg |> 
            tidylog::select(-NAME)
            )
